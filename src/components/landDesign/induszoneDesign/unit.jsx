...
const IndusUnit = () => {
  ...
  const [indusUnitData, setindusUnitData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);
  const api = `${apiUrl}/city/filterCityAuthUser?size=Unit`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setindusUnitData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setindusUnitData(resp?.data.data);
