...
const Aribalarge = () => {
  ...
  const [aribaLargeData, setAribaLargeData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const api = `${apiUrl}/asset/filterLandSize/verified?size=Large`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setAribaLargeData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setAribaLargeData(resp?.data.data);
