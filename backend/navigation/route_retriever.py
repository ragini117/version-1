...
# Make sure backend root is available in sys.path
sys.path.insert(
    0,
...
class RouteRetriever:
    ...
    def search_routes(
        ...
        """
        Search navigation routes from Qdrant.

        Returns the top-k semantically relevant routes
        ...
        try:
            ...
            logger.error(
                f"Error during semantic route search: {exc}"
            )
            return []