...
const Props = () => {
  ...
  const handleGetdata = async (modelId) => {
    setLoading(true);
    try {
      const api = `${apiUrl}/props/getPropAssets/${modelId}`;
  ...
  const handleProductDetails = (value) => {
    ...
    router.push(`/nftmarketdashboard/props/productdetail?${params.toString()}`);
  };
                ...
                <div className="col-12 col-md-3">
                  <div className={`${styles.card_new}`}>
                    <div className={` ${styles.card_new_img}`}>
                      <img
                        src={`${baseUrl}/asset/getImages?imageName=${value?.imageUrl[0]}&pathName=${value?.imagePath}`}
                        alt="land-img"
                      />
                    </div>
