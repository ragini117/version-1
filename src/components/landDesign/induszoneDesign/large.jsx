...
const IndusLarge = () => {
  ...
  const [indusLargeData, setindusLargeData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);
  const api = `${apiUrl}/city/filterCityAuthUser?size=Large`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setindusLargeData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setindusLargeData(resp?.data.data);
