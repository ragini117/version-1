...
const Aribamega = () => {
  ...
  const [aribaMegaData, setAribaMegaData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const api = `${apiUrl}/asset/filterLandSize/verified?size=Mega`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setAribaMegaData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setAribaMegaData(resp?.data.data);
