...
const Aribaunit = () => {
  ...
  const [aribaUnitData, setAribaUnitData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);
  // const api = "/asset/filterLandSize?size=Unit";
  const api = `${apiUrl}/asset/filterLandSize/verified?size=Unit`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setAribaUnitData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setAribaUnitData(resp?.data.data);
  ...
  const handleProductDetails = (value) => {
    ...
    router.push(`/marketdashboard/land/productdetail?${params.toString()}`);
  };
