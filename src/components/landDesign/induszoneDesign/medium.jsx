...
const IndusMedium = () => {
  ...
  const [indusMediumData, setindusMediumData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);
  const api = `${apiUrl}/city/filterCityAuthUser?size=Medium`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setindusMediumData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setindusMediumData(resp?.data.data);
