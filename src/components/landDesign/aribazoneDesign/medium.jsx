...
const Aribamedium = () => {
  ...
  const [aribaMediumData, setAribaMediumData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const api = `${apiUrl}/asset/filterLandSize/verified?size=Medium`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setAribaMediumData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setAribaMediumData(resp?.data.data);
