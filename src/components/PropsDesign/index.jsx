...
const Props = () => {
  ...
  const [propsAllData, setpropsAllData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const handleGetdata = async (modelId) => {
    setLoading(true);
    try {
      const api = `${apiUrl}/props/getVerifiedPropAssets/${modelId}`;
      const res = await axios.get(api);
      if (res.status === 200) {
        setpropsAllData(res?.data?.results);
  ...
  const handlePageClick = async (event) => {
    ...
    try {
      ...
      setpropsAllData(resp?.data?.results);
  ...
  const handleProductDetails = (value) => {
    ...
    params.append("propId", value?.propId?.toString());
    params.append("category", value?.category?.toString());
    params.append("modelId", value?.modelId?.toString());
    router.push(`/marketdashboard/props/productdetail?${params.toString()}`);
  };
      ...
      <div className={`${styles.content}`}>
        ...
        <div className="container">
          <div className="row g-3">
            ...
            {propsAllData?.map((value, key) => {
              return (
                <div className="col-12 col-md-3">
                  <div className={`${styles.card_new}`}>
                    <div className={` ${styles.card_new_img}`}>
                      <img
                        src={`${baseUrl}/asset/getImages?imageName=${value?.imageUrl[0]}&pathName=PROPS_IMAGE`}
                        alt="land-img"
                      />
                    </div>
