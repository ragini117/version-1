...
from utils.openai_client import extract_message_content, get_openai_client
...
class DomainService:
    ...
    def needs_live_data(self, query: str) -> bool:
        q = query.lower()
        return any(kw in q for kw in ["price", "current price", "worth", "value", "market cap", "trading", "volume", "listed", "listing", "exchange", "latest news", "today", "recent update"])
    ...
    def check_domain(self, query: str):
        if self.is_greeting(query):
            return {"is_domain": True, "confidence": 1.0}

        q_lower = query.lower()
        if any(keyword in q_lower for keyword in DOMAIN_KEYWORDS):
            return {"is_domain": True, "confidence": 0.95}
        ...
        try:
            ...
            q = q.lower()
            is_domain = any(topic in q for topic in DOMAIN_KEYWORDS)
            return {"is_domain": is_domain, "confidence": 0.8 if is_domain else 0.0}
