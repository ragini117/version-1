...
const ModelMega = () => {
  ...
  const [modelMegaData, setmodelMegaData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const api = `${apiUrl}/asset/filterModelSizeWise/verified?size=Mega`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setmodelMegaData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setmodelMegaData(resp?.data.data);
  ...
  const handleProductDetails = (value) => {
    ...
    router.push(`/marketdashboard/model/productdetail?${params.toString()}`);
  };
          ...
          <div className="col-12">
            <h5 className={`${styles.title_text}`}>
              ...
              <span className={`ml-2 p-2 ${styles.total_count}`}>
                mega ( {pagination?.totalItems} )
              </span>
            </h5>
          </div>
          {modelMegaData?.map((value, key) => {
            return (
