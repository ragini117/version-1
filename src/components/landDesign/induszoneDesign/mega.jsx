...
const IndusMega = () => {
  ...
  const [indusMegaData, setindusMegaData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);
  const api = `${apiUrl}/city/filterCityAuthUser?size=Mega`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setindusMegaData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setindusMegaData(resp?.data.data);
