...
const ModelMedium = () => {
  ...
  const [modelMediumData, setmodelMediumData] = useState([]);
  const [pagination, setPagination] = useState({});
  const [loading, setLoading] = useState(false);

  const api = `${apiUrl}/asset/filterModelSizeWise/verified?size=Medium`;
  const handleGetdata = async () => {
    setLoading(true);
    try {
      const res = await axios.get(api);
      if (res.status === 200) {
        setmodelMediumData(res?.data?.data);
  ...
  const handlePageClick = async (event) => {
    try {
      ...
      setmodelMediumData(resp?.data.data);
          ...
          <div className="col-12">
            <h5 className={`${styles.title_text}`}>
              ...
              <span className={`ml-2 p-2 ${styles.total_count}`}>
                medium ( {pagination?.totalItems} )
              </span>
            </h5>
          </div>
          {modelMediumData?.map((value, key) => {
            return (
